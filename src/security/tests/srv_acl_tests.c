 ...
 */
static char *TEST_USER = "myuser@";
static char *TEST_GROUP = "mygroup@";

/*
 * Test helper functions
 */
static Auth__Token *
create_valid_auth_token(char *user, char *grp,
			char *grp_list[], size_t num_grps)
{
	Auth__Token	*token;
	Auth__Sys	*authsys;
...
static void
init_valid_cred(d_iov_t *cred, char *user, char *grp,
		char *grp_list[], size_t num_grps)
{
			      ...
			      uint64_t requested_capas)
{
	struct daos_acl	*acl;
	d_iov_t		cred;
	char		*grps[] = { "badgroup@", TEST_GROUP, "worsegroup@" };
...
expect_no_list_access_with_perms(uint64_t acl_perms, uint64_t requested_capas)
{
	...
	d_iov_t			cred;
	struct pool_owner	ownership;
	char			*grps[] = { "wronggroup@", TEST_GROUP };
